...
<body class="hold-transition sidebar-mini layout-fixed layout-navbar-fixed">
    <main class="wrapper">
        ...
        require ADMIN . 'includes/sidebar.inc.php' ?>
            ...
            <div class="content-header"> 
                <a href='#'   class='btn btn-sm btn-primary openmod' >Manage Awards</a>
            </div>
                            ...
                            <table class='table table-striped table-hover'>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td><?= $i ?></td>
                                        <td>  
                                        <a href='/awards/contestants/<?= $row['a_url'] ?>' target='_blank' style='color:blue' ><?= $row['a_title'] ?></a></td>
                                        <td> <?= $row['user_full_name']?> </td> 
                                        <td> <img src='/public/assets/uploads/<?= $row['a_img'] ?>' style='height:40px' > </td>
                                        <td> <?= $row['a_status'] ?> </td>
                                      
                                        <td>
                                            <a class='btn btn-sm btn-danger drop_awards' href='#'   rel='<?= $row['a_ID'] ?>'>Delete</a> 
                                        </td> 
  ...
  <script>
    $(document).ready(function () {
             ...
             $('.drop_awards').click(function(e) {
                e.preventDefault();
                var form = new FormData();
                form.append('id', $(this).attr('rel') );  
                
                if (confirm("Are you sure?")) {
                    const data = _data(form, 'drop_awards');
